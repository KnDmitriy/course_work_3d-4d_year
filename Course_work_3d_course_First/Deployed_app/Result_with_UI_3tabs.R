...
#   female_names_rus <- read.csv("./Data/female_names_rus.txt", header=FALSE)
#   male_names_rus <- read.csv("./Data/male_names_rus.txt", header=FALSE)
#   male_surnames_rus <- read.csv("./Data/male_surnames_rus.txt", header=FALSE)
...
#     req(file)
#     input_data <- read.xlsx(file$datapath, sheetIndex = 1, header = FALSE)
#     load_stopwords()
...
#     # tmp <- str_replace_all(x$lemma, paste("\\b(", stopwords_combined, ")\\b"), "") # без этого остается частоповтояряющееся слово "правительство"
...
#     View(preprocessed_texts_word_list)
...
#     # View(list(files_preprocessed_data[["df_1"]], files_preprocessed_data[["df_2"]], files_preprocessed_data[["df_3"]]))
#     d_all <- Filter(Negate(is.null), list(files_preprocessed_data[["df_1"]], files_preprocessed_data[["df_2"]], files_preprocessed_data[["df_3"]])) 
#     View(names(files_preprocessed_data)[1])
#     for (i in 1:3) {
#         names(d_all) <- c("word",
#                              paste0("freq_", names(files_preprocessed_data)[i], sep = ""), # names(files_preprocessed_data) == list('df1', 'df2', 'df3')
#                              paste0("tf_", names(files_preprocessed_data)[i], sep = ""))
#     }
#     cos.mat <- NULL
#     if (length(d_all) == 1) {
#     }
...
#     if (length(d_all) == 3) {
#       View(d_all)
#       d_all <- join_all(d_all, by = 'word', type = 'full', match = "all")
...
# # 
# #       tf_idf <- d_all %>%
# #         mutate(num_of_occurrences = rowSums(d_all)) # tf_idf <- select(d_all, 'word', 'freq.x', 'tf.x', 'freq.y','tf.y', 'freq', 'tf')
# #       tdm_df <- select(all_regions_tf_idf_df, 'word', c(grep('^freq', names(all_files_data))))  # tdm_df <- select(d_all, 'word', 'freq.x', 'freq.y', 'freq')
# #       tdm_df <- tdm_df %>% mutate(num_of_occurrences = rowSums(select(tdm_df, 'freq1', 'freq2', 'freq3') != 0))  #  tdm_df <- tdm_df %>% mutate(num_of_occurrences = rowSums(select(tdm_df, 'freq1', 'freq2', 'freq3') != 0))
# #       tdm_df <- tdm_df %>% mutate(idf = log(4 / (1 + num_of_occurrences) + 1))
# #       tf_idf <- tf_idf %>% mutate(num_of_occurrences = tdm_df$num_of_occurrences)
# #       tf_idf <- tf_idf %>% mutate(idf = tdm_df$idf)
# # 
# #       # extra
# #       tf_idf_with_word_tf__idf <- select(tf_idf, 'word', c(grep('^tf', names(tf_idf)), 'idf'))
# # 
# # 
# #       for (i in seq_along(names(all_files_data))) {
# #         tf_idf_with_word_tf__idf[[paste0("tf_idf_",  names(all_files_data)[[i]], sep = "")]] <- tf_idf_with_word_tf__idf[[i + 1]] * tf_idf_with_word_tf__idf$idf
# #       }
# #       tf_idf_only <- select(tf_idf_with_word_tf__idf, grep('^tf_idf', names(tf_idf_with_word_tf__idf)))
